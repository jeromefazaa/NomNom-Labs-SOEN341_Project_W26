...
import {
  ...
  resetMealPlan,
  saveMealPlan,
} from "../../../redux/slices/mealPlannerSlice.js";
...
function MainPage() {
  ...
  const mealPlan = useSelector((state) => state.mealPlanner.mealPlan) ?? [];
  ...
  const filteredRecipes = recipes
    ...
    .filter(({ recipe }) => {
      const searchMatch =
        recipe.title.toLowerCase().includes(processedSearch) ||
        recipe.difficulty.toLowerCase().includes(processedSearch) ||
        recipe.diet.toLowerCase().includes(processedSearch);
      ...
      const timeMatch =
        timeFilter === "All"
          ...
          : timeFilter === "gt60"
            ? recipe.prepTime > 60
            : recipe.prepTime <= Number(timeFilter);
      ...
      const costMatch =
        costFilter === "All"
          ...
          : costFilter === "gt50"
            ? recipe.cost > 50
            : recipe.cost <= Number(costFilter);

      return (
        searchMatch && timeMatch && difficultyMatch && dietMatch && costMatch
      );
    });
  ...
  const handleMealSelection = (day, meal, selectedRecipeTitle) => {
    dispatch(updateMeal({ day, meal, recipeTitle: selectedRecipeTitle }));
  };
  ...
  const recipeBeingEdited =
    formMode === "edit" && editingRecipeIndex !== null
